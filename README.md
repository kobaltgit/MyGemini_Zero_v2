...
9. [Настройка защищённого WebApp (GitHub Pages)](#-настройка-защищённого-webapp-github-pages)
...
## 🖥️ Деплой на сервере (Linux / systemd)
...
## 🌐 Настройка защищённого WebApp (GitHub Pages)

Чтобы ввод мастер-пароля открывался как всплывающее модальное окно с точками `••••••`:
...
3. В выпадающем меню выберите:
   ...
   * **Folder:** `/docs` (папка уже подготовлена в проекте)
   * Нажмите **Save**.
4. Скопируйте полученную ссылку (например, `https://username.github.io/MyGemini_Zero_v2/`) и укажите её в `.env`:
   ```ini
